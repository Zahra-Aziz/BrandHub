...
#query to get textSearch of products from db 
query = {'textSearch': {'$exists': 1}}
projection = {'_id': 0,'textSearch':1}

#query to get IDs of products from db 
query1 = {'PId': {'$exists': 1}}
...
def ConvertToList(string): #converts string to list
...
def TextSearchGender(gender): #this function returns textsearch for specified gender from db
    s=0 
    i=0
    titles=[] #to store IDs(titles) of products
    text=[] #to store textSearch of products
    ...
    if (gender=='F'):
        ...
        gen="Men" #specify gender 
    print(brands)
    s=0 
    i=0
    id_count=0
    for b in brands: #iterate through all brands for a specific gender
        brand=b
...
def TextSearch(inputText,searchText,titles,gender,array_idx,brands): #this function computes the textual search and retrieves results of search from db
    ...
    fromdocs=process.extract(inputText, choices_dict,scorer=fuzz.token_set_ratio,limit=10) #apply fuzzy search. get Maximum of 30 products

    number=0
    dict1={} #this will contain all resultant products of textSearch
    product={}  #this will contain individual product of textSearch
...
api.add_resource(SearchByText,"/sbt/<string:searchInput>/<string:gender>") #takes 2 input: text for search & gender
